defmodule Aelita2.ProjectControllerTest do
  ...
  test "list linked projects", %{conn: conn, project: project} do
    conn = login conn
    user = Repo.get_by!(User, user_xref: 23)
    Repo.insert! %LinkUserProject{user_id: user.id, project_id: project.id}
    conn = get conn, project_path(conn, :index)
    assert html_response(conn, 200) =~ "example/project"
  end

  test "show an unbatched patch", %{conn: conn, project: project} do
    conn = login conn
  ...
  test "show a batched patch", %{conn: conn, project: project} do
    conn = login conn
